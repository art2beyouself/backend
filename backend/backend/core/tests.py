...
from backend.core.external import verify_email
from backend.core.views.auth import enrich_user
...
class ExternalTestCase(TestCase):
    @patch("backend.core.external.call_external_api", Patches.call_external_api_hunter)
    ...
    @patch("backend.core.external.call_external_api", Patches.call_external_api_none)
    @patch("django.conf.settings.EMAIL_HUNTER_API_KEY", "123")
    def test_notVerifyEmail(self):
        self.assertFalse(verify_email("diver@gmail.com"))

    @patch("backend.core.external.call_external_api", Patches.call_external_api_clearbit)
...
class AuthTestCase(TestCase):
    @patch("backend.core.views.auth.verify_email", lambda email: True)
    @patch("backend.core.views.auth.enrich_email", lambda email: {})
